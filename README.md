...
3. 按提示输入信息：
   ...
   - **附件放置位置**:
     - 输入 `1`：附件统一放在外层 `wiz/media`
     - 输入 `2`：附件放在对应笔记同目录下的 `media/`
     - 直接回车默认使用 `1`
...
A: 请按你的导出模式一起复制附件目录：
- 模式 1：确保 `wiz` 和 `wiz/media` 一起复制。图片会按笔记所在目录自动生成指向外层 `wiz/media` 的相对路径。
- 模式 2：确保每个笔记目录下的 `media/` 一起复制，图片使用相对路径 `media/...` 引用。
